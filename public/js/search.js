...
_vue2.default.directive('dialog', {
	bind: function bind() {}
});

_vue2.default.filter('round', function (value, decimals) {
	if (!value) {
		value = 0;
	}
...
var paginator = _vue2.default.extend({
	...
	methods: {
		fetch: function fetch() {
			...
			this.$http.get(this.url + '?query=' + this.query.trim() + '&page=' + page).then(function (response) {
				console.log(response.data);
				_this.resource = JSON.parse(response.data);
				_this.makePagination(_this.resource);
			}, function (response) {
				// error callback
			});
		},
...
new _vue2.default({
	...
	methods: {
		...
		search_container: function search_container() {
			this.$broadcast('search', 'container');
		},
		search_cargo: function search_cargo() {
			this.$broadcast('search', 'cargo');
		},
		search_manifest: function search_manifest() {
...
},{"moment":1,"vue":4,"vue-resource":3}]},{},[5]);
