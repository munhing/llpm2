...
<div class="portlet box blue-hoki">
		...
		<tbody>
			@foreach($portusers as $user)
				<tr>
                    ...
                    <td>{{ $user->company }}</td>
                    <td>
                        {{ $user->is_staff == '0' ? '<span class="label label-sm label-warning">PENDING</span>' : '' }}
                        {{ $user->is_staff == '2' ? '<span class="label label-sm label-success">ENABLED</span>' : '' }}
                        {{ $user->is_staff == '3' ? '<span class="label label-sm label-danger">DISABLED</span>' : '' }}

                    </td>
					<td>{{ $user->created_at }}</td>
					<td>
						<button class="btn btn-xs btn-default" data-toggle="modal" data-target="#myModal_user" data-user-id="{{ $user->id }}" data-name="{{ $user->name }}" data-username="{{ $user->username }}" data-email="{{ $user->email }}" data-company="{{ $user->company }}">
                        ...
                        @if($user->is_staff == '0')
                            {{ Form::open(['route'=>'portusers_access.approve']) }}
                                {{ Form::hidden('portuser_id', $user->id) }}
                                <button class='btn btn-default btn-xs' data-confirm>
                                    <i class="fa fa-check-square-o"></i> Approve 
                                </button>
                            {{ Form::close() }}
                        @endif  
                        @if($user->is_staff == '2')
                            {{ Form::open(['route'=>'portusers_access.disable']) }}
                                {{ Form::hidden('portuser_id', $user->id) }}
                                <button class='btn btn-default btn-xs' data-confirm>
                        ...
                        @endif   
                        @if($user->is_staff == '3')
                            {{ Form::open(['route'=>'portusers_access.enable']) }}
                                {{ Form::hidden('portuser_id', $user->id) }}
                                <button class='btn btn-default btn-xs' data-confirm>
    ...
    {{ Form::open(['route'=>['portusers_access.update'], 'id' => 'form_user_update']) }}	
